...
func New(_ context.Context, next http.Handler, config *Config, name string) (http.Handler, error) {
	...
	config.KeycloakIssuerURL = strings.TrimRight(strings.TrimSpace(config.KeycloakIssuerURL), "/")
	config.AnythingLLMBaseURL = strings.TrimRight(strings.TrimSpace(config.AnythingLLMBaseURL), "/")
	config.KeycloakClientID = strings.TrimSpace(config.KeycloakClientID)
	config.KeycloakUsernameClaim = strings.TrimSpace(config.KeycloakUsernameClaim)

	switch {
	case config.KeycloakIssuerURL == "":
		return nil, errors.New("keycloakIssuerURL is required")
	case config.KeycloakClientID == "":
		return nil, errors.New("keycloakClientId is required")
	case keycloakClientSecret == "":
		return nil, errors.New("client secret is required")
	case config.AnythingLLMBaseURL == "":
	...
	config.CallbackPath = ensureLeadingSlash(config.CallbackPath)
	config.LoginPath = ensureLeadingSlash(config.LoginPath)
	config.LogoutPath = ensureLeadingSlash(config.LogoutPath)
	config.AnythingLLMLogoutDetectionPath = ensureLeadingSlash(config.AnythingLLMLogoutDetectionPath)
	...
	return &Middleware{
		...
		stateCookieName:      config.SessionCookieName + "_state",
	}, nil
}
...
func (m *Middleware) handleLogin(rw http.ResponseWriter, req *http.Request) {
	if _, ok := m.readSession(req); ok {
		http.Redirect(rw, req, m.publicURL(req, "/"), http.StatusFound)
		return
	}
...
func (m *Middleware) startLogin(rw http.ResponseWriter, req *http.Request) {
	...
	http.Redirect(rw, req, m.keycloakAuthURL(m.publicURL(req, m.config.CallbackPath), nonce), http.StatusFound)
}
...
func (m *Middleware) handleCallback(rw http.ResponseWriter, req *http.Request) {
	...
	if !ok || state.Nonce != stateToken {
		m.writeError(rw, http.StatusUnauthorized, "invalid or expired login state")
		return
	}

	tokenResponse, err := m.exchangeCode(req.Context(), code, m.publicURL(req, m.config.CallbackPath))
	...
	http.Redirect(rw, req, m.publicURL(req, loginPath), http.StatusFound)
}
...
func (m *Middleware) handleLogout(rw http.ResponseWriter, req *http.Request) {
	...
	query.Set("post_logout_redirect_uri", m.publicURL(req, "/"))

	http.Redirect(rw, req, m.keycloakEndpoint("/protocol/openid-connect/logout")+"?"+query.Encode(), http.StatusFound)
}
...
func (m *Middleware) exchangeCode(ctx context.Context, code, redirectURI string) (*KeycloakExchangeTokenResponse, error) {
	...
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, m.keycloakEndpoint("/protocol/openid-connect/token"), strings.NewReader(form.Encode()))
...
func (m *Middleware) fetchUserInfo(ctx context.Context, accessToken string) (map[string]any, error) {
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, m.keycloakEndpoint("/protocol/openid-connect/userinfo"), nil)
...
func (m *Middleware) keycloakEndpoint(path string) string {
	return m.config.KeycloakIssuerURL + path
}

func (m *Middleware) keycloakAuthURL(redirectURI, state string) string {
	query := url.Values{}
	...
	return m.keycloakEndpoint("/protocol/openid-connect/auth") + "?" + query.Encode()
}
...
func (m *Middleware) callAnythingLLM(ctx context.Context, method, path string, body, target any) error {
	...
	request, err := http.NewRequestWithContext(ctx, method, m.anythingLLMEndpoint(path), reader)
...
func (m *Middleware) anythingLLMEndpoint(path string) string {
	return m.config.AnythingLLMBaseURL + path
}
...
func (m *Middleware) readSession(req *http.Request) (*SessionPayload, bool) {
	...
	if payload.UsernameClaim != m.config.KeycloakUsernameClaim {
		return nil, false
	}
...
func (m *Middleware) publicURL(req *http.Request, path string) string {
	scheme := "http"
